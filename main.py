...
# ПОЛНЫЙ СПИСОК ГОЛОСОВ (Ничего не вырезано)
VOICES = {
...
async def process_pay(message: types.Message):
    conn = sqlite3.connect(DB_PATH); conn.execute('UPDATE users SET is_premium = 1 WHERE user_id = ?', (message.from_user.id,)); conn.commit(); conn.close()
...
async def set_voice(call: types.CallbackQuery):
    ...
    await call.message.answer(f"✅ Голос установлен: {v_id}"); await call.answer()
...
# --- 6. FASTAPI МАРШРУТЫ (С ПОЛНЫМ REQUEST ДЛЯ РОУТОВ) ---
app = FastAPI()
...
async def home(request: Request):
    ...
    return templates.TemplateResponse(request, "index.html", {"request": request, "posts": all_posts[:15]})
...
async def blog_list(request: Request):
    ...
    all_posts = [dict(p) for p in db_posts] + BLOG_POSTS
    return templates.TemplateResponse(request, "blog_index.html", {"request": request, "posts": all_posts, "is_single": False})

@app.post("/api/admin/generate-post")
async def api_admin_gen(req: AdminGenRequest):
    try:
        # ЖЕСТКИЙ SEO ПРОМПТ ДЛЯ HTML РАЗМЕТКИ
        prompt = f"""
        Напиши SEO-статью на тему: "{req.message}".
        Используй ТОЛЬКО HTML теги: <h2>, <h3>, <p>, <ul>, <li>, <strong>.
        Никакого Markdown (без # и **). 
        
        СТРОГИЙ ФОРМАТ ОТВЕТА:
        TITLE: [Заголовок статьи]
        EXCERPT: [SEO описание 160 знаков]
        CONTENT: [HTML текст со всеми подзаголовками и списками]
        """
        raw_res = await mm.generate(prompt)
        
        # Парсинг с защитой от пустых полей
        title = re.search(r"TITLE:(.*?)(?=EXCERPT|$)", raw_res, re.S).group(1).strip()
        excerpt = re.search(r"EXCERPT:(.*?)(?=CONTENT|$)", raw_res, re.S).group(1).strip()
        content = raw_res.split("CONTENT:")[1].strip()
        
        new_slug = slugify(title)
        img_url = "https://images.unsplash.com/photo-1677442136019-21780ecad995?q=80&w=1024" # ИИ картинка
...
async def voices_page(request: Request):
    # Страница "Голоса"
    return templates.TemplateResponse(request, "voices.html", {"request": request, "voices": VOICES})

@app.get("/about", response_class=HTMLResponse)
async def about_page(request: Request):
    # Страница "О нас" / "О проекте"
    return templates.TemplateResponse(request, "about.html", {"request": request})

@app.get("/instructions", response_class=HTMLResponse)
async def instructions_page(request: Request):
    # Страница "Инструкция"
    return templates.TemplateResponse(request, "instructions.html", {"request": request})

@app.get("/faq", response_class=HTMLResponse)
async def faq_page(request: Request):
    # Страница "Помощь"
    return templates.TemplateResponse(request, "faq.html", {"request": request})
...
async def disclaimer_page(request: Request):
    # Страница "Отказ от ответственности"
...
async def wait_page(request: Request, file: str, key: str = None):
    if key and key.upper() in [k.upper() for k in PREMIUM_KEYS]:
        ...
        if os.path.exists(p): return FileResponse(path=p, filename="speech.mp3")
...
async def chat_api(req: ChatRequest):
    return {"reply": await mm.generate(req.message)}
