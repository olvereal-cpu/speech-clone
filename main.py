...
async def api_admin_gen(req: AdminGenRequest):
    conn = None
    try:
        # Улучшенный SEO промпт
        prompt = f"""
        Напиши SEO-статью на тему: "{req.message}". 
        Используй строго HTML теги: <h2>, <p>, <ul>, <li>. 
        ...
        if "Ошибка" in raw_res: return JSONResponse(status_code=500, content={"message": raw_res})
        ...
        try:
            ...
            title, excerpt, content = req.message, "AI News", raw_res.replace("```html", "").replace("```", "")
        ...
        cursor.execute('''INSERT INTO posts 
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''', 
            (title, new_slug, "https://images.unsplash.com/photo-1614064641935-4476e83bb023", 
             excerpt, content, datetime.now().strftime("%d.%m.%Y"), 
...
async def read_post(request: Request, slug: str):
    ...
    return templates.TemplateResponse(request, "blog_index.html", {"request": request, "posts": [post], "is_single": True})
...
async def wait_page(request: Request, file: str, key: str = None):
    if key and key.upper() in [k.upper() for k in PREMIUM_KEYS]:
        ...
        if os.path.exists(p): return FileResponse(path=p, filename="speechclone.mp3")
