...
def clean_audio(): audio_dir = "static/audio" 
    if os.path.exists(audio_dir): 
for filename in os.listdir(audio_dir): file_path = os.path.join(audio_dir, filename) try: 
if os.path.isfile(file_path): os.unlink(file_path) 
    except Exception as e: print(f"Error cleaning: {e}")
...
async def get_blog_post(request: Request, post_name: str): post_path = f"blog/{post_name}.html" 
...
@app.get("/ads.txt") 
async def get_ads_txt(): if os.path.exists("ads.txt"): 
    return FileResponse("ads.txt") 
    raise HTTPException(status_code=404, detail="ads.txt not found")

@app.post("/api/generate") 
async def generate(request: TTSRequest): if not request.text or len(request.text) > 2000: 
